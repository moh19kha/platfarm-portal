          ...
          {sel&&<div className="dp">
            ...
            <div style={{padding:"16px 18px"}}>
              ...
              {selType==="rcv"&&detTab===1&&<div>
                ...
                {(sel.att||[]).length>0?(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{(sel.att||[]).map((a,i)=><div key={i} style={{borderRadius:6,overflow:"hidden",border:"1px solid #E4E1DC",aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",background:"#F7F6F3"}}><div style={{textAlign:"center"}}><div style={{fontSize:16}}>{a.t==="photo"?"📷":"📄"}</div><div style={{fontSize:8,color:"#95A09C",marginTop:2,padding:"0 4px",lineHeight:1.3}}>{a.n}</div></div></div>)}</div>):(<div style={{padding:16,textAlign:"center",borderRadius:8,background:"#F7F6F3",border:"1px solid #E4E1DC",color:"#95A09C",fontSize:11}}>No photos uploaded</div>)}
              </div>}
              ...
              {selType==="trf"&&detTab===1&&<div>
                ...
                {(()=>{const loadAtt=(sel.att||[]).filter(a=>{const pt=a.pt||"";const arrQc=["arrival","bale_condition","arrival_weighbridge","tarp_damage","bale_cross_section","moisture_reading","nir_reading"];return !arrQc.includes(pt);});return loadAtt.length>0?(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{loadAtt.map((a,i)=><div key={i} style={{borderRadius:6,overflow:"hidden",border:"1px solid #E4E1DC",aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",background:"#F7F6F3"}}><div style={{textAlign:"center"}}><div style={{fontSize:16}}>{a.t==="photo"?"📷":"📄"}</div><div style={{fontSize:8,color:"#95A09C",marginTop:2,padding:"0 4px",lineHeight:1.3}}>{a.n}</div></div></div>)}</div>):(<div style={{padding:16,textAlign:"center",borderRadius:8,background:"#F7F6F3",border:"1px solid #E4E1DC",color:"#95A09C",fontSize:11}}>No loading photos uploaded</div>)})()}
              </div>}
