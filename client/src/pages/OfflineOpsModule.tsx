...
export default function OfflineOpsModule(){
  ...
  const [viewPhoto, setViewPhoto] = useState<{irAttId:number,label:string}|null>(null);
  ...
  useEffect(() => {
    if (sel && sel.att) {
      for (const a of sel.att) {
        if (a.irAttId && a.t === "photo") loadImg(a.irAttId);
      }
    }
  }, [sel, loadImg]);

  const openPhoto = useCallback((irAttId: number, label: string) => {
    if (!irAttId) return;
    setViewPhoto({ irAttId, label });
    loadImg(irAttId);
  }, [loadImg]);

  const downloadPhoto = useCallback((irAttId: number, fileName: string) => {
    const src = imgCache[irAttId];
    if (!src || src === "loading" || src === "none") return;
          ...
          <div className="xc"><div className="xh" style={{background:"#475577"}}><h3><ImgIcon src={ICON_TRANSFER} size={18} /> Dakhla → Sokhna Transfers</h3><span className="ct">{fTrf.length}</span></div>
            <table className="t"><thead><tr><th>ID</th><th>Commodity</th><th>Grade</th><th>Stage</th><th>Bales</th><th>Weight</th><th>Truck / Plate</th><th>Driver</th><th>Seal</th><th>Load Date</th><th>Status</th></tr></thead><tbody>
              {fTrf.map(r=>{const st=TRS[r.status]||TRS.in_transit;return(<tr key={r.id} onClick={()=>openDet(r,"trf")} style={{background:sel&&sel.id===r.id?"#E4EFE6":""}}>
                ...
                <td>{(()=>{const s2=r.status==="received"||r.status==="assessed";const s3=s2&&!!r.qcData;const s1=!!(r.loadDate||r.status==="in_transit");const stg=s3?"Quality Check":s2?"Confirmed Receive":s1?"Shipped / In Transit":"Draft";const clr=s3?"#2D5A3D":s2?"#4A7C59":s1?"#D4960A":"#95A09C";const bg=s3?"#E4EFE6":s2?"#E4EFE6":s1?"#FDF6EC":"#F7F6F3";const num=s3?3:s2?2:s1?1:0;return <span style={{display:"inline-flex",padding:"2px 8px",borderRadius:99,fontSize:9,fontWeight:600,background:bg,color:clr,whiteSpace:"nowrap",gap:4}}><span>{num}/4</span>{stg}</span>})()}</td>
                <td className="m" style={{fontSize:10}}>{r.bales}</td>
          ...
          {sel&&<div className="dp">
            ...
            <div style={{padding:"16px 18px"}}>
              ...
              {selType==="rcv"&&detTab===1&&<div>
                ...
                {(sel.att||[]).length>0?(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{(sel.att||[]).map((a,i)=><div key={i} onClick={()=>a.irAttId&&openPhoto(a.irAttId,a.n)} style={{borderRadius:6,overflow:"hidden",border:"1px solid #E4E1DC",aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",background:imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?"transparent":"#F7F6F3",cursor:a.irAttId?"pointer":"default",position:"relative"}}>{imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?(<><img src={imgCache[a.irAttId]} alt={a.n} style={{width:"100%",height:"100%",objectFit:"cover",position:"absolute",inset:0}}/><div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent,rgba(0,0,0,0.7))",padding:"12px 4px 4px",fontSize:7,color:"#fff",textAlign:"center",lineHeight:1.3}}>{a.n}</div></>):(<div style={{textAlign:"center"}}><div style={{fontSize:16}}>{a.t==="photo"?"📷":"📄"}</div><div style={{fontSize:8,color:"#95A09C",marginTop:2,padding:"0 4px",lineHeight:1.3}}>{a.n}</div></div>)}</div>)}</div>):(<div style={{padding:16,textAlign:"center",borderRadius:8,background:"#F7F6F3",border:"1px solid #E4E1DC",color:"#95A09C",fontSize:11}}>No photos uploaded</div>)}
              </div>}
              ...
              {selType==="dpr"&&detTab===0&&<div>
                ...
                {[{icon:"\u25b6\ufe0f",title:"Shift Started",sub:sel.startTime||"\u2014",color:"#C0714A"},{icon:"🏭",title:"Pressing",sub:sel.line+" \u2014 "+sel.commodity+(sel.shift?" ("+sel.shift+")":""),color:"#C0714A"},{icon:"📦",title:"Output",sub:sel.outBales+" bales \u2014 "+fK(sel.outWeight)+" (avg "+sel.outAvgBale+" kg)",color:"#2D5A3D"},{icon:"\u23f9\ufe0f",title:"Shift Ended",sub:sel.endTime||"\u2014",color:"#C0714A"},{icon:sel.sync==="synced"?"\u2601\ufe0f":"\u23f3",title:sel.sync==="synced"?"Synced to Odoo":"Pending Sync",sub:sel.date,color:sel.sync==="synced"?"#2D5A3D":"#D4960A"}].filter(Boolean).map((ev,i)=><div key={i} style={{display:"flex",gap:12,marginBottom:0}}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                    <div style={{width:28,height:28,borderRadius:14,background:ev.color+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>{ev.icon}</div>
                    {i<4&&<div style={{width:2,height:24,background:"#E8E5E0"}}/>}
                  </div>
              ...
              {selType==="trf"&&detTab===1&&<div>
                ...
                {(()=>{const loadAtt=(sel.att||[]).filter(a=>{const pt=a.pt||"";const arrQc=["arrival","bale_condition","arrival_weighbridge","tarp_damage","bale_cross_section","moisture_reading","nir_reading"];return !arrQc.includes(pt);});return loadAtt.length>0?(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{loadAtt.map((a,i)=><div key={i} onClick={()=>a.irAttId&&openPhoto(a.irAttId,a.n)} style={{borderRadius:6,overflow:"hidden",border:"1px solid #E4E1DC",aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",background:imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?"transparent":"#F7F6F3",cursor:a.irAttId?"pointer":"default",position:"relative"}}>{imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?(<><img src={imgCache[a.irAttId]} alt={a.n} style={{width:"100%",height:"100%",objectFit:"cover",position:"absolute",inset:0}}/><div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent,rgba(0,0,0,0.7))",padding:"12px 4px 4px",fontSize:7,color:"#fff",textAlign:"center",lineHeight:1.3}}>{a.n}</div></>):(<div style={{textAlign:"center"}}><div style={{fontSize:16}}>{a.t==="photo"?"📷":"📄"}</div><div style={{fontSize:8,color:"#95A09C",marginTop:2,padding:"0 4px",lineHeight:1.3}}>{a.n}</div></div>)}</div>)}</div>):(<div style={{padding:16,textAlign:"center",borderRadius:8,background:"#F7F6F3",border:"1px solid #E4E1DC",color:"#95A09C",fontSize:11}}>No loading photos uploaded</div>)})()}
              </div>}
              ...
              {selType==="trf"&&detTab===3&&<div>
                ...
                {sel.qcData?(<div>
                  ...
                  {(()=>{const qcAtt=(sel.att||[]).filter(a=>["bale_cross_section","moisture_reading","nir_reading"].includes(a.pt));return qcAtt.length>0?(<div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>{qcAtt.map((a,i)=><div key={i} onClick={()=>a.irAttId&&openPhoto(a.irAttId,a.pt==="bale_cross_section"?"Bale Cross Section":a.pt==="moisture_reading"?"Moisture Reading":a.pt==="nir_reading"?"NIR Reading":a.n)} style={{borderRadius:8,overflow:"hidden",border:"1px solid #D5E5D5",background:imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?"transparent":"#F2F7F3",padding:imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?0:10,textAlign:"center",cursor:a.irAttId?"pointer":"default",position:"relative",aspectRatio:"4/3"}}>{imgCache[a.irAttId]&&imgCache[a.irAttId]!=="loading"&&imgCache[a.irAttId]!=="none"?(<><img src={imgCache[a.irAttId]} alt={a.n} style={{width:"100%",height:"100%",objectFit:"cover",position:"absolute",inset:0,borderRadius:8}}/><div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent,rgba(0,0,0,0.7))",padding:"16px 6px 6px",borderRadius:"0 0 8px 8px"}}><div style={{fontSize:10,fontWeight:600,color:"#fff"}}>{a.pt==="bale_cross_section"?"Bale Cross Section":a.pt==="moisture_reading"?"Moisture Reading":a.pt==="nir_reading"?"NIR Reading":a.n}</div></div></>):(<><div style={{fontSize:20,marginBottom:4}}>{a.pt==="moisture_reading"?"💧":a.pt==="nir_reading"?"📊":"🌾"}</div><div style={{fontSize:10,fontWeight:600,color:"#2D5A3D"}}>{a.pt==="bale_cross_section"?"Bale Cross Section":a.pt==="moisture_reading"?"Moisture Reading":a.pt==="nir_reading"?"NIR Reading":a.n}</div><div style={{fontSize:9,color:"#95A09C",marginTop:2}}>{a.n}</div></>)}</div>)}</div>):(<div style={{padding:12,textAlign:"center",borderRadius:8,background:"#F7F6F3",border:"1px solid #E4E1DC",color:"#95A09C",fontSize:11}}>No QC photos captured</div>)})()}
