// Standalone page for viewing a Sales Shipment in a new tab (via /sales/:id)
...
export default function SalesShipmentPage() {
  ...
  const { data: shipment } = trpc.salesShipments.getById.useQuery(
    { id: shipmentId },
  ...
  useEffect(() => {
    if (shipment?.name) {
      ...
      document.title = "Sales Shipment - Platfarm";
    }
