...
export default function FinanceModule(){
  ...
  const[soaF,setSoaF]=useState(()=>{const d=new Date();d.setMonth(d.getMonth()-6);return d.toISOString().slice(0,10);});
    ...
    <div style={{fontFamily:"'DM Sans', system-ui, sans-serif",background:"#F7F6F3",minHeight:"100vh"}}>
      ...
      <div className="mob-pb-bar" style={{marginLeft:sw,transition:"margin-left .2s",minHeight:"calc(100vh - 3px)",overflow:isMob?"auto":undefined}}>
        ...
        {pg==="receivables"&&(rcvQ.isLoading?<PageSkeleton/>:rcvQ.isError?<SectionError/>:rcvQ.data?<div style={{padding:20}}>
          ...
          <div className="xc"><div className="xh" style={{background:"#C0714A"}}><h3>🔴 Overdue</h3></div><div className="mob-table-scroll"><table className="tbl"><thead><tr><th>Invoice</th><th>Customer</th><th style={{textAlign:"right"}}>Amount</th><th>Due</th><th>Age</th><th>Risk</th></tr></thead><tbody>{r.overdue.map(o=>(<tr key={o.id}><td className="mn" style={{color:"#2D5A3D",fontSize:11}}>{o.ref}</td><td style={{fontSize:11}}>{o.customer}</td><td className="mn" style={{textAlign:"right",fontSize:11}}>{fmt(o.amount,cur)}</td><td className="mn" style={{fontSize:11}}>{o.dueDate}</td><td className="mn" style={{fontSize:11,color:o.daysOverdue>90?"#C94444":"#D4960A"}}>{o.daysOverdue}d</td><td><Badge type={o.risk} label={o.risk}/></td></tr>))}</tbody></table></div></div>
          </>);})()}
        </div>:<div style={{padding:40,textAlign:"center",color:"#95A09C"}}>No data available</div>)}
