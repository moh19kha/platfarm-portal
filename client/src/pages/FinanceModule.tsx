    ...
    <div style={{fontFamily:"'DM Sans', system-ui, sans-serif",background:"#F7F6F3",minHeight:"100vh"}}>
      ...
      <div className="mob-pb-bar" style={{marginLeft:sw,transition:"margin-left .2s",minHeight:"calc(100vh - 3px)",overflow:isMob?"auto":undefined}}>
        ...
        {pg==="receivables"&&(rcvQ.isLoading?<PageSkeleton/>:rcvQ.isError?<SectionError/>:rcvQ.data?<div style={{padding:20}}>
          {(()=>{const r=rcvQ.data;return(<>
          <div style={{fontSize:10,color:"#95A09C",marginBottom:12}}>Aging as of <strong style={{color:"#2C3E50"}}>{todayLabel}</strong></div>
          <div className="sg" style={{gridTemplateColumns:"repeat(5,1fr)"}}>{[["Total AR",fmt(r.total,cur),""],["DSO",r.dso+"d",""],["Collection",r.collectionRate+"%",""],["Bad Debt",fmt(r.badDebt,cur),"provision"],["Concentration",pct(r.topConcentration),r.topConcentration>25?<Badge type="danger" label="High"/>:<Badge type="low" label="OK"/>]].map(([l,v,s],i)=>(<div className="sc" key={i}><div className="sl">{l}</div><div className="sv" style={{fontSize:17}}>{v}</div><div className="ss">{s}</div></div>))}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
            <div className="ip"><h4>Aging Buckets</h4>{[["Current 0-30d",r.aging.current,"#2D5A3D"],["31-60d",r.aging.d31,"#D4960A"],["61-90d",r.aging.d61,"#C0714A"],["90+d",r.aging.d90,"#C94444"]].map(([l,d,c])=>(<div key={l} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:11,color:"#64706C"}}>{l} ({d.count})</span><span className="mn" style={{fontSize:11,color:c}}>{fmt(d.amount,cur)} · {d.pct}%</span></div><PB v={d.pct} color={c}/></div>))}</div>
            <div className="ip"><h4>Customer Concentration</h4>{r.topCustomers.map((c,i)=>(<div key={i} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><div style={{display:"flex",alignItems:"center",gap:6}}><RD l={c.risk}/><span style={{fontSize:11,fontWeight:500}}>{c.name}</span></div><span className="mn" style={{fontSize:11,color:c.share>25?"#C94444":"#2D5A3D"}}>{pct(c.share)}</span></div><PB v={c.share} color={c.share>25?"#C94444":c.share>15?"#D4960A":"#2D5A3D"} h={4}/></div>))}</div>
          </div>
          {(()=>{const filteredOverdue=rcvSearch?r.overdue.filter(o=>{const q=rcvSearch.toLowerCase();return (o.ref||"").toLowerCase().includes(q)||(o.customer||"").toLowerCase().includes(q)||(o.dueDate||"").includes(q);}):r.overdue;return(<div className="xc"><div className="xh" style={{background:"#C0714A",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}><h3>🔴 Overdue ({filteredOverdue.length}{rcvSearch?" filtered":""})</h3><input value={rcvSearch} onChange={e=>setRcvSearch(e.target.value)} placeholder="Search invoice, customer…" style={{padding:"4px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.3)",background:"rgba(255,255,255,0.15)",color:"#fff",fontSize:11,width:220,outline:"none"}} /></div><div className="mob-table-scroll"><table className="tbl"><thead><tr><th>Invoice</th><th>Shipment</th><th>Customer</th><th style={{textAlign:"right"}}>Amount</th><th>Due</th><th>Age</th><th>Risk</th></tr></thead><tbody>{filteredOverdue.map(o=>(<tr key={o.id}><td className="mn" style={{fontSize:11}}><a href={`https://odoo.platfarm.io/web#id=${o.id}&model=account.move&view_type=form`} target="_blank" rel="noreferrer" style={{color:"#2D5A3D",textDecoration:"none",fontWeight:500}} onMouseEnter={e=>e.currentTarget.style.textDecoration="underline"} onMouseLeave={e=>e.currentTarget.style.textDecoration="none"}>{o.ref}</a></td><td className="mn" style={{fontSize:11}}>{o.soRef?<a href={`/sales/${o.soId||""}`} target="_blank" rel="noreferrer" style={{color:"#4A7C59",textDecoration:"none",fontWeight:500}} onMouseEnter={e=>e.currentTarget.style.textDecoration="underline"} onMouseLeave={e=>e.currentTarget.style.textDecoration="none"}>{o.soRef}</a>:<span style={{color:"#C4C0BB"}}>—</span>}</td><td style={{fontSize:11}}>{o.customer}</td><td className="mn" style={{textAlign:"right",fontSize:11}}>{fmt(o.amount,cur)}</td><td className="mn" style={{fontSize:11}}>{o.dueDate}</td><td className="mn" style={{fontSize:11,color:o.daysOverdue>90?"#C94444":"#D4960A"}}>{o.daysOverdue}d</td><td><Badge type={o.risk} label={o.risk}/></td></tr>))}{filteredOverdue.length===0&&<tr><td colSpan={6} style={{padding:20,textAlign:"center",color:"#95A09C",fontSize:11}}>No results for "{rcvSearch}"</td></tr>}</tbody></table></div></div>);})()}
          </>);})()}
        </div>:<div style={{padding:40,textAlign:"center",color:"#95A09C"}}>No data available</div>)}
