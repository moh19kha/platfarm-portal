    ...
    <div style={{fontFamily:"'DM Sans', system-ui, sans-serif",background:"#F7F6F3",minHeight:"100vh"}}>
      ...
      <div className="mob-pb-bar" style={{marginLeft:sw,transition:"margin-left .2s",minHeight:"calc(100vh - 3px)",overflow:isMob?"auto":undefined}}>
        ...
        {pg==="receivables"&&(rcvQ.isLoading?<PageSkeleton/>:rcvQ.isError?<SectionError/>:rcvQ.data?<div style={{padding:20}}>
          ...
          {(()=>{const filteredOverdue=rcvSearch?r.overdue.filter(o=>{const q=rcvSearch.toLowerCase();return (o.ref||"").toLowerCase().includes(q)||(o.customer||"").toLowerCase().includes(q)||(o.soRef||"").toLowerCase().includes(q)||(o.dueDate||"").includes(q);}):r.overdue;return(<div className="xc"><div className="xh" style={{background:"#C0714A"}}><h3>🔴 Overdue ({filteredOverdue.length}{rcvSearch?" matching":""})</h3></div><div className="mob-table-scroll"><table className="tbl"><thead><tr><th>Invoice</th><th>Shipment</th><th>Customer</th><th>Payment Term</th><th>Ref Date</th><th style={{textAlign:"right"}}>Amount</th><th>Due</th><th>Age</th><th>Risk</th></tr></thead><tbody>{filteredOverdue.map(o=>(<tr key={o.id}><td className="mn" style={{fontSize:11}}><a href={`https://odoo.platfarm.io/web#id=${o.id}&model=account.move&view_type=form`} target="_blank" rel="noreferrer" style={{color:"#2D5A3D",textDecoration:"none",fontWeight:500}} onMouseEnter={e=>e.currentTarget.style.textDecoration="underline"} onMouseLeave={e=>e.currentTarget.style.textDecoration="none"}>{o.ref}</a></td><td className="mn" style={{fontSize:11}}>{o.soRef?<a href={`/sales/${o.soId||""}`} target="_blank" rel="noreferrer" style={{color:"#4A7C59",textDecoration:"none",fontWeight:500}} onMouseEnter={e=>e.currentTarget.style.textDecoration="underline"} onMouseLeave={e=>e.currentTarget.style.textDecoration="none"}>{o.soRef}</a>:<span style={{color:"#C4C0BB"}}>—</span>}</td><td style={{fontSize:11}}>{o.customer}</td><td style={{fontSize:11,color:"#64706C"}}>{o.paymentTerm||"—"}</td><td className="mn" style={{fontSize:11,color:"#64706C"}}>{o.refDate||"—"}</td><td className="mn" style={{textAlign:"right",fontSize:11}}>{fmt(o.amount,cur)}</td><td className="mn" style={{fontSize:11}}>{o.dueDate}</td><td className="mn" style={{fontSize:11,color:o.daysOverdue>90?"#C94444":"#D4960A"}}>{o.daysOverdue}d</td><td><Badge type={o.risk} label={o.risk}/></td></tr>))}{filteredOverdue.length===0&&<tr><td colSpan={8} style={{padding:20,textAlign:"center",color:"#95A09C",fontSize:11}}>No results for "{rcvSearch}"</td></tr>}</tbody></table></div></div>);})()}
          </>);})()}
        </div>:<div style={{padding:40,textAlign:"center",color:"#95A09C"}}>No data available</div>)}
